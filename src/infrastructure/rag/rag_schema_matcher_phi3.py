...
1. [CRITICAL] __init__ used to silently instantiate a hardcoded
   BaseLLMClient(model="phi3:mini", ...) whenever `llm_client is None`,
...
2. [CRITICAL] Dead class-level attribute
   `llm_client = BaseLLMClient(model="phi3:mini", temperature=0.1)` sitting
...
class RAGSchemaMatcher:
    ...
    def __init__(
        ...
        self._llm_client = BaseLLMClient(model="phi3:mini", temperature=0.1)  # None means "no LLM validation" -- honoured now.
