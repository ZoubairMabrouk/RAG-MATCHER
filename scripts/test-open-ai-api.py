#!/usr/bin/env python3
"""
Dynamic RAG Virtual Rename runner (Batched LLM Version)

This script:
1) Loads U-Schema (JSON) from --uschema-file (or stdin)
2) Introspects current relational schema via DI container
3) Builds semantic KB and indexes it in FAISS
4) Runs RAG-based batched matching:
   - entities -> tables
   - attributes -> columns
5) Computes migration plan via DiffEngine (virtual renames)
6) Emits JSON mapping report and prints SQL statements
"""
...
from src.domain.entities.schema import USchema, USchemaEntity, USchemaAttribute, DataType, SchemaMetadata
...
from src.infrastructure.rag.once_schme_matcher import RAGSchemaMatcher
    ...
    if kb_file and Path(kb_file).exists():
        ...
        with open(kb_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # Convert JSON to KnowledgeBaseDocument
                kb_doc = KnowledgeBaseDocument(
                    id=doc_json.get("id", ""),
                    table=doc_json.get("table", "*"),
                    column=doc_json.get("column", "*"),
...
def run(args) -> int:
    ...
    # 4) Batched semantic mapping
    entities_for_batch = [
        {"name": e.name, "attributes": [a.name for a in e.attributes]}
        for e in uschema.entities
    ]
    mapping_report_entities = matcher.match_all_entities(entities_for_batch)
    ...
    # 6) Build SQL (no renames generated)
    builder = MigrationBuilder(args.dialect)
    sql_statements = builder.build_migration(changes)

    # 7) Print summary
    log.info("\n=== Semantic Mapping Summary ===")
    for emap in mapping_report_entities:
        ent = emap["entity"]
        tgt = emap.get("matched_table") or "(new table)"
        conf = emap.get("table_confidence", 0.0)
        log.info(f"- {ent} -> {tgt} (conf={conf:.3f})")
        for attr in emap.get("attributes", []):
            col = attr.get("target_column") or "(new column)"
            log.info(f"    · {attr['name']} -> {col} (conf={attr.get('confidence', 0.0):.3f})")
    ...
    if args.out:
        ...
        payload = {
            ...
            "mapping": mapping_report_entities,
...
# -------------------- CLI --------------------
def parse_args():
    p = argparse.ArgumentParser(description="Dynamic RAG virtual rename runner (batched LLM)")
    p.add_argument("--uschema-file", default="./uschema.json", help="Path to U-Schema JSON")
    ...
    p.add_argument("--table-threshold", type=float, default=0.35, help="Accept threshold for table matching")
    p.add_argument("--column-threshold", type=float, default=0.35, help="Accept threshold for column matching")
    p.add_argument("--top-k", type=int, default=5, help="Top-K candidates to retrieve")
    p.add_argument("--out", default=None, help="Optional path to write JSON report")
    p.add_argument("--kb-file", default="./data/rag/knowledge_base_enriched.jsonl", help="Optional external KB file")
    return p.parse_args()
