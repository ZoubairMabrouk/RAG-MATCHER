...
def load_uschema(uschema_json: Dict[str, Any]) -> USchema:
    ...
    for e in uschema_json.get("uSchemaModel", {}).get("entities", []):
        ...
        for variation in et.get("variations", []):
            ...
            for attr_block in sv.get("properties", []):
                if "Attribute" in attr_block:
                    for a in attr_block["Attribute"]:
                        if isinstance(a, str):
                            continue
                        aname = a["name"]
                        atype = _to_datatype(a["type"])
                        iskey = a.get("iskey", False)
                        attributes.append(USchemaAttribute(name=aname, data_type=atype, is_key=iskey))
...
def build_kb_and_index(matcher: RAGSchemaMatcher, schema: SchemaMetadata):
...
def run(args) -> int:
    ...
    build_kb_and_index(matcher, current_schema)
